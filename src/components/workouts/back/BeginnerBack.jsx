...
function BeginnerBack() {
  ...
  useEffect(() => {
    fetch("http://localhost:5173/data/workouts/back/Beginner.json")
      .then((res) => {
        if (!res.ok) {
  ...
  return (
    <>
      <Heading value={"Back trainng for beginners"} />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
        {abs.map((item) => (
          <div key={item.id} className="rounded-lg shadow-2xl overflow-hidden">
            {item ? (
              <img
                src={item.image}
                alt={item.name}
        ...
        {showModal && (
          <>
            <Modal
              video={
                <iframe className="w-full h-[300px]" src={modalContent.video} />
              }
